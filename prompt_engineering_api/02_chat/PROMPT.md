...
Filename 02_chat.py. Extend the hello example into a terminal chat. Preserve multi-turn context using the simplest current Responses API continuation mechanism. Print a complete response only after it arrives. Support quit and exit. Make the state variable visible so students can identify it.

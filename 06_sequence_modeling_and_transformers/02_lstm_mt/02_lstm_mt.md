...
Concept: gating. Compare this file line-by-line against `01_rnn_mt.py` --
...
Exactly what `01_rnn_mt.py` does, with one architectural change. See that
...
`01_rnn_mt.output.txt`: every one of the five evaluation sentences scores a
lower (better) reference loss here, including the deliberately long one --
the same task, same data, one architectural change.

## Honest caveats

Same caveats as `01_rnn_mt.md` apply: this is a small, from-scratch model on

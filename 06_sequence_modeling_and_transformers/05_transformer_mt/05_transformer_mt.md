...
`03_rnn_mt.py` and `04_lstm_mt.py` -- but there is no hidden state carried
...
Look at the final validation loss compared to `03_rnn_mt.output.txt` and
`04_lstm_mt.output.txt`: it keeps improving, RNN → LSTM → Transformer, with
...
Same caveats as `03_rnn_mt.md`: small from-scratch model, noisy data, no

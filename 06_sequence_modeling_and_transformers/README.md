...
`pages/` holds the working copies of the four Canvas lesson pages (6.1
Module Overview, 6.2 RNNs, 6.3 LSTMs, 6.4 Transformers/Attention/RLHF),
built from this module's real, captured demo output. `discussion/` holds
the working copies of the 6.5 and 6.6 discussion prompts, rewritten to ask
students to reason about the specific numbers and examples in 6.2/6.3's
demo output rather than generic RNN/LSTM trivia. See `DESIGN-module-6.md`
for the design brief these were authored from (written when this module
was still numbered 7; renumbered to 6 to close a numbering gap).

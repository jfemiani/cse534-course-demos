...
\begin{itemize}
    \item<1-> \textbf{Autoregressive:} Output at each step depends on the model's own previous outputs
    \item<2-> Every generative model needs $P(\text{next} \mid \text{context})$
    \item<3-> Understanding N-gram math = understanding \textbf{all} language models
    \item<4-> Same probability foundations power both simple and complex models
\end{itemize}
\end{frame}

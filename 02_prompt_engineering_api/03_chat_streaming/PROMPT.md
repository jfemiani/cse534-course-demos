...
Filename 03_chat_streaming.py. Change the preceding multi-turn chat to use the current Responses API streaming interface. Print only text-delta events as they arrive and capture the completed response identifier for the next turn. Keep all other behavior the same.
